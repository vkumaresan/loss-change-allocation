...
def add_classification_losses(model, input_labels, l2=0):
    ...
    with tf.name_scope('losses'):
        ...
        model.a('cross_ent', tf.nn.softmax_cross_entropy_with_logits(logits=model.logits, labels=input_labels, name='cross_ent'))
        model.a('loss_cross_ent', tf.reduce_mean(model.cross_ent, name='loss_cross_ent'), trackable=True)
        model.a('prediction_correct', tf.equal(tf.argmax(model.prob,1), tf.argmax(input_labels,1), name='prediction_correct'))
